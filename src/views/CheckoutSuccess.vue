...
const messages = ref([
  ...
  {
    from: 'you',
    message: 'you validate the payment process.',
    color: 'success',
    icon: 'mdi-check',
  },
  {
    from: 'You',
    message: 'Payment accepted, you have the access to the tour.',
    color: 'success',
...
onMounted(async ()=>{
try {
  const response = await axios.post('api/v1/bookings/bookingCheckout', {
    tour: tourId.value,
    user: user.value,
    price: price.value
  });
...
<template>
    <h1 class="mx-5 my-8 text-success">
        <v-icon color="success" icon="mdi-certificate"/>
        Congratutions you have joined the tour
    </h1>
    <h2 class="mx-5 mb-8 font-100 text-subtitle-1"> you be redirected to home page in 10 seconds </h2>
    ...
    <v-card-text class="text-body-1" v-text="tour.summary" />
      ...
      <v-card-actions>
        <v-btn color="success" size="large" variant="outlined" class="ma-5" :to="{name:'homePage'}">Go to home page </v-btn>
    </v-card-actions>
    </v-card>
</template>